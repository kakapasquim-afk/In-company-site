...
- **Fotografias reais** de turmas, campi, eventos e equipe — sensação de instituição viva e regional; `object-cover` com zoom suave no hover. **Exceção (2026-09-14):** fotos panorâmicas muito largas que perderiam conteúdo relevante com `cover` (ex.: `sescoop-4`, `sescoop-5` no duo do Sescoop) usam exibição integral `object-contain`/`height: auto` via classe escopada - nunca cortar.
